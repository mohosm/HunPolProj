...
class App extends Component {
  ...
  componentWillMount(){
    ...
    for(let i = 1; i < Object.keys(g).length+1; i++){
      let cObj = {id: g[i].id, title: g[i].title, vane: false}
      contBasic.push(cObj);
 ...
 for(var i in copy){
  ...
  render() {
    ...
    let cont1=cont0.map(c=>{
      if (c.vane === false){
      return <p key = {c.id} className={c.title+"menu"}>{c.title}</p>
      }else {
    return <Link to={"/"+c.id.toString()} key={c.id} className={c.title+"menu"}>{c.title}</Link>
  }
});
